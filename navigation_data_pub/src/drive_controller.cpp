...
nav_msgs::Odometry odom; // Holds odometry information
geometry_msgs::Twist cmdVel; // Velocity command
geometry_msgs::PoseStamped desired; // Desired waypoint

// Declare constants
const double PI = 3.141592;
const double Ka = 0.65; // Proportional constant for angular velocity 
const double Klv = 0.95; // Proportional constant for linear velocity 
const double initialX = 0.0;
const double initialY = 0.0;
const double angularTolerance = 0.15; // Radians
const double distanceTolerance = 0.0001; // Meters
const double MIN_LINEAR_VEL = 0.5; // meters per second
const double MAX_LINEAR_VEL = 0.6; // meters per second
...
void update_pose(const nav_msgs::Odometry &currentOdom) {
  ...
  odom.pose.pose.orientation.z = currentOdom.pose.pose.orientation.z; // radians
}
...
void set_velocity() {
  ...
  if (location_met && abs(final_desired_heading_error) < .15) {
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber subCurrentPose = node.subscribe("odom_data_euler", 10, update_pose, ros::TransportHints().tcpNoDelay());
