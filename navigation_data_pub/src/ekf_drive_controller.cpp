...
geometry_msgs::Twist twist; // Velocity command
...
const double angularTolerance = 0.09; // Radians
...
void set_velocity() {
  ...
  if (abs(angularError) >= angularTolerance) {
    ...
    twist.angular.z = Ka * angularError;
    twist.linear.x = 0;
  }
    ...
    twist.linear.x = Klv * getDistanceError();
    twist.angular.z = 0;
  }
  else {
...
int main(int argc, char **argv) {
  ...
  ros::Rate loop_rate(10);
    ...
    //cout << "cmd_vel = " << twist.linear.x <<" ,  "<<twist.angular.z<< endl;
    loop_rate.sleep();
  }
  return 0;
}
